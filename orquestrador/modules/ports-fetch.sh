...
: "${PORTS_VERIFY_GPG:=false}"                  # verificar .asc se disponível
...
catalog::_record_file(){
  ...
  local rec
  if (( have_jq )); then
    rec=$(jq -nc --arg url "$url" --arg path "$path" --arg sha "$sha" \
               --arg size "$size" --arg mtime "$mtime" --arg etag "$etag" \
               '{url:$url, path:$path, sha256:$sha, size:($size|tonumber?), mtime:$mtime, etag:$etag}')
  else
    rec="{\"url\":\"$url\",\"path\":\"$path\",\"sha256\":\"$sha\",\"size\":$size,\"mtime\":\"$mtime\",\"etag\":\"$etag\"}"
  fi

  # lock do catálogo
  lock::named_acquire "catalog" 0
  # índices simples (tsv)
  printf "%s\t%s\n" "$url" "$path" | __append_atomic "$CAT_URL_IDX"
  printf "%s\t%s\n" "$sha" "$path" | __append_atomic "$CAT_SHA_IDX"
  # jsonl catálogo
  printf "%s\n" "$rec" | __append_atomic "$CAT_DB_JSON"
  lock::named_release "catalog"
}
...
fetch::batch(){
  ...
  export -f fetch::get_file fetch::_curl_one fetch::verify_sha256 fetch::verify_gpg_if_enabled \
            ...
            __ensure_dir __sha256_file __ts __sanitize __atomic_write __append_atomic \
            ...
            PORTS_VERIFY_GPG have_gpg have_jq CACHE_DIR META_DIR CATALOG_DIR CAT_URL_IDX CAT_SHA_IDX CAT_DB_JSON
